...
export function SettingsModal({ categories, focusCategories = false, onCategoriesChange, onClose }: { categories: CategoryDefinition[]; focusCategories?: boolean; onCategoriesChange: (value: CategoryDefinition[]) => void; onClose: () => void }) {
  const { language, font, setLanguage, setFont, t } = useSettings()
  const [newCategory, setNewCategory] = useState('')
  const [subcategories, setSubcategories] = useState<Record<string,string>>({})
  const categorySection = useRef<HTMLElement>(null)
  useEffect(() => { if (focusCategories) window.setTimeout(() => categorySection.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50) }, [focusCategories])
